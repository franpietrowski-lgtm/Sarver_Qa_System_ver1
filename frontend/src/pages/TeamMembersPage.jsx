...
/* ── Timeline selector periods ── */
const TIMELINES = [1, 3, 6, 12, 24];

/* ── Hover Stats (above card) ── */
function HoverStats({ profileId }) {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: 8 }}
      className="absolute bottom-full left-1/2 z-[60] mb-3 w-60 -translate-x-1/2 rounded-[18px] border border-[var(--border)] bg-[var(--card)] p-3 shadow-2xl"
    ...
    >
      ...
      {/* Arrow pointer */}
      <div className="absolute left-1/2 top-full -translate-x-1/2 border-8 border-transparent border-t-[var(--card)]" />
    </motion.div>
  );
}

/* ── Org Node for Hierarchy ── */
...
/* ── PROFILE DETAIL OVERLAY — glass backdrop, fully opaque card ── */
function ProfileOverlay({ profile, onClose, onAvatarDone }) {
  ...
  if (d.source_type === "crew" || d.source_type === "member") {
    profileLinks.push({ label: "Training Sessions", href: "/training" });
  }
  if (d.auth_role === "management" || d.auth_role === "owner") {
    profileLinks.push({ label: "Calibration Heatmap", href: "/analytics" });
    profileLinks.push({ label: "Reviewer Performance", href: "/reviewer-performance" });
  }
  profileLinks.push({ label: "Repeat Offenders", href: "/repeat-offenders" });

  return (
    <div
      className="fixed inset-0 z-[100] flex items-center justify-center"
      onClick={onClose}
      data-testid="profile-detail-overlay"
    >
      {/* Glass backdrop — heavy blur + dark overlay */}
      ...
      <motion.div
        ...
        className="relative z-10 w-full max-w-lg overflow-hidden rounded-[28px] bg-[var(--card)] shadow-2xl ring-1 ring-[var(--border)]"
        onClick={e => e.stopPropagation()}
              ...
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                ...
                {timelineStats ? (
                  <div className="grid grid-cols-2 gap-2 pt-1" data-testid="profile-stats-grid">
                    {[["review_count","Reviews"],["submission_count","Submissions"],["avg_review_score","Avg Score"],["training_completed","Trained"]].map(([k,l])=>(
...
function CrewTeamCard({ team, onCardClick, scale = 1 }) {
  ...
  return (
    <div className="flex flex-col items-center" style={{ minWidth: 180 * scale, maxWidth: 280 * scale }}>
      <div className="rounded-[18px] px-5 py-3 text-center shadow-md ring-1 ring-[var(--border)] w-full bg-[var(--card)]">
      ...
      <button type="button" onClick={() => onCardClick(team.lead)} className="flex w-full items-center gap-3 rounded-[18px] bg-[var(--card)] px-4 py-2.5 shadow ring-1 ring-[var(--border)] transition hover:shadow-lg">
        <div style={{ padding: 2, clipPath: HEX_CLIP, backgroundColor: a.bar }}>
      ...
      {team.members.length > 0 && (
        <>
          ...
          <div className="w-full space-y-1.5">
            {team.members.map(m => (
              <button key={m.profile_id} type="button" onClick={() => onCardClick(m)} className="flex w-full items-center gap-2.5 rounded-[14px] bg-[var(--card)] px-3 py-2 text-left shadow-sm ring-1 ring-[var(--border)] transition hover:ring-2" data-testid={`team-member-${m.profile_id}`}>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {m.division && <p className="text-[8px] text-[var(--muted-foreground)]">{m.division}</p>}
                </div>
              </button>
            ))}
          </div>
...
function TeamStructureView({ teams, onCardClick }) {
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(() => {
      ...
      const needed = count * 240;
      setScale(needed > w && count > 1 ? Math.max(0.7, w / needed) : 1);
    });
    observer.observe(el);
...
/* ═══════ DIVISION HIERARCHY — matches user's exact org chart ═══════
   OWNER → GM
                |  |____ ZACH → Install crews
   Megan __|  |____ TIM → Maintenance crews
   Daniel__|  |____ SCOTT → Maintenance crews
   Scott K_|  |____ BRAD → Tree crews → PHC
*/
function DivisionHierarchyView({ hierarchy, onCardClick }) {
  if (!hierarchy) return null;
  const pmA = accent("Production Manager");
  const amA = accent("Account Manager");
  const clA = accent("Crew Leader");
  const cmA = accent("Crew Member");

  return (
    <div className="h-full overflow-y-auto pb-8" data-testid="hierarchy-view">
      <div className="flex flex-col items-center">
        {/* ── OWNER ── */}
        <div className="flex flex-wrap justify-center gap-4">
          {hierarchy.owners.map(p => <OrgCard key={p.profile_id} profile={p} onClick={onCardClick} />)}
        </div>
        <VLine h={24} color={accent("Owner").bar} />

        {/* ── GM ── */}
        <div className="flex flex-wrap justify-center gap-4">
          {hierarchy.general_managers.map(p => <OrgCard key={p.profile_id} profile={p} onClick={onCardClick} />)}
        </div>
        <VLine h={20} color={accent("GM").bar} />

        {/* ── GM SPLIT: AMs (left) | PMs → Divisions (right) ── */}
        <div className="relative flex w-full items-start justify-center gap-8 lg:gap-16">

          {/* Account Managers — left column */}
          <div className="flex flex-col items-center w-48 lg:w-56 shrink-0">
            <p className="rounded-xl px-4 py-1.5 text-xs font-bold text-white" style={{ backgroundColor: amA.bar }}>Account Managers</p>
            <VLine h={10} color={amA.bar} dashed />
            <div className="space-y-2 w-full">
              {hierarchy.account_managers.map(p => (
                <button key={p.profile_id} type="button" onClick={() => onCardClick(p)} className="flex w-full items-center gap-2 rounded-[14px] bg-[var(--card)] px-3 py-2 shadow-sm ring-1 ring-[var(--border)] transition hover:shadow-md" data-testid={`hierarchy-am-${p.profile_id}`}>
                  <div style={{ padding: 2, clipPath: HEX_CLIP, backgroundColor: amA.bar }}>
                    <HexAvatar name={p.name} url={p.avatar_url} size={32} />
                  </div>
                  <div className="min-w-0 flex-1 text-left">
                    <p className="truncate text-xs font-semibold text-[var(--foreground)]">{p.name}</p>
                    <p className="text-[8px] font-bold uppercase" style={{ color: amA.text }}>Account Manager</p>
                  </div>
                </button>
              ))}
            </div>
          ...
          <div className="flex flex-col items-center shrink-0">
            <div className="h-full min-h-[200px] w-[2px]" style={{ backgroundColor: accent("GM").bar }} />
          </div>

          {/* Production Managers → Divisions — right side */}
          <div className="flex flex-col gap-5 flex-1 min-w-0">
            {hierarchy.divisions.map(div => (
              <div key={div.name} className="flex items-start gap-3" data-testid={`hierarchy-division-${div.name.toLowerCase()}`}>
                {/* PM badge(s) for this division */}
                <div className="flex flex-col gap-1 shrink-0 w-44 lg:w-52">
                  {(div.production_managers || []).map(pm => (
                    <button key={pm.profile_id} type="button" onClick={() => onCardClick(pm)} className="flex items-center gap-2 rounded-[14px] bg-[var(--card)] px-3 py-2 shadow-sm ring-1 ring-[var(--border)] transition hover:shadow-md" data-testid={`hierarchy-pm-${pm.profile_id}`}>
                      <div style={{ padding: 2, clipPath: HEX_CLIP, backgroundColor: pmA.bar }}>
                        <HexAvatar name={pm.name} url={pm.avatar_url} size={32} />
                      </div>
                      <div className="min-w-0 flex-1 text-left">
                        <p className="truncate text-xs font-semibold text-[var(--foreground)]">{pm.name}</p>
                        <p className="text-[8px] font-bold uppercase" style={{ color: pmA.text }}>PM</p>
                      </div>
                    </button>
                  ))}
                </div>
                ...
                <div className="flex items-center self-center shrink-0">
                  <div className="h-[2px] w-6 lg:w-10" style={{ backgroundColor: pmA.bar }} />
                  <div className="border-4 border-transparent" style={{ borderLeftColor: pmA.bar, borderLeftWidth: 6 }} />
                </div>

                {/* Division pill + teams */}
                <div className="flex flex-col items-start gap-2 flex-1 min-w-0">
                  <span className="rounded-lg px-3 py-1 text-[10px] font-bold text-white" style={{ backgroundColor: clA.bar }}>{div.name}</span>
                  {div.teams.map(team => (
                    <div key={team.lead.profile_id} className="ml-2 flex flex-col">
                      <button type="button" onClick={() => onCardClick(team.lead)} className="flex items-center gap-2 rounded-[12px] bg-[var(--card)] px-3 py-1.5 shadow-sm ring-1 ring-[var(--border)] transition hover:shadow-md">
                        <div style={{ padding: 2, clipPath: HEX_CLIP, backgroundColor: clA.bar }}>
                          <HexAvatar name={team.lead.name} url={team.lead.avatar_url} size={28} />
                        </div>
                        <div className="text-left">
                          <p className="text-[11px] font-semibold text-[var(--foreground)]">{team.lead.name}</p>
                          <p className="text-[8px] text-[var(--muted-foreground)]">{team.lead.crew_label || ""}</p>
                        </div>
                      </button>
                      {team.members.length > 0 && (
                        <div className="ml-8 mt-1 space-y-1">
                          {team.members.map(m => (
                            <button key={m.profile_id} type="button" onClick={() => onCardClick(m)} className="flex items-center gap-2 rounded-[10px] bg-[var(--card)] px-2 py-1 shadow-sm ring-1 ring-[var(--border)] transition hover:shadow" data-testid={`hierarchy-member-${m.profile_id}`}>
                              <div style={{ padding: 1, clipPath: HEX_CLIP, backgroundColor: cmA.bar }}>
                                <HexAvatar name={m.name} url={m.avatar_url} size={22} />
                              </div>
                              <p className="text-[10px] font-medium text-[var(--foreground)]">{m.name}</p>
                              {m.division && m.division !== div.name && (
                                <span className="rounded px-1 py-0.5 text-[7px] font-bold uppercase" style={{ backgroundColor: accent("Crew Member").card, color: accent("Crew Member").text }}>{m.division}</span>
                              )}
                            </button>
                          ))}
                        </div>
...
export default function TeamMembersPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] flex-col" data-testid="team-members-page">
      ...
      <AnimatePresence>
        {selected && <ProfileOverlay profile={selected} onClose={() => setSelected(null)} onAvatarDone={() => load()} />}
      </AnimatePresence>
    </div>
  );
}
