...
import { authDownload, authGet, authPost } from "@/lib/api";
...
export default function ExportsPage() {
  ...
  const downloadExport = async (item) => {
    try {
      const response = await authDownload(`/exports/${item.id}/download`);
      const blobUrl = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement("a");
      link.href = blobUrl;
      link.download = `${item.dataset_type}-${item.id}.${item.export_format}`;
